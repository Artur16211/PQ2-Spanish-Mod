        ...
        for key, value in mod_msgs_dict.items():
            if value is not None:
                ...
                if not value.startswith("# "):
                    def insert_n_character(line):
                        target_position = 43
                        if len(line) <= 43 or len(line) <= 45:
                            return line

                        left_space = line.rfind(' ', 0, target_position)
                        right_space = line.find(' ', target_position)
                        #
                        left_brak = line.rfind('[', 0, target_position)
                        right_brak = line.find(']', target_position)

                        # check if the target position is inside a bracket
                        if left_brak != -1 and right_brak != -1:
                            if left_brak == -1 or right_brak == -1 or left_brak > right_brak:
                                insert_position = target_position
                            else:
                                insert_position = right_brak + 1
...
root.mainloop()
>>>>>>> 2b1e48e75a1a4254b3a1441c7aca29004c744938
