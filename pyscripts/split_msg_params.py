...
from get_params import only_params
import logging
...
def process_input_file(input_path, output_path, max_length):
    base_name = os.path.basename(input_path)
    base_name_no_ext = os.path.splitext(base_name)[0]
    base_name_no_bmd = base_name_no_ext.replace('.bmd', '').replace('.BMD', '')

    output_filename = os.path.join(output_path, f"{base_name_no_bmd}.msg")
...
def output_params(input_path, output_path):
    base_name = os.path.basename(input_path)
    base_name_no_ext = os.path.splitext(base_name)[0]
    base_name_no_bmd = base_name_no_ext.replace('.bmd', '').replace('.BMD', '')

    params_filename = os.path.join(output_path, f"{base_name_no_bmd}.msg")
    ...
    for line in lines:
        if line.startswith('[msg') or line.startswith('[sel'):
            params_only_list.append(line)
        else:
            cleaned_line = re.sub(r'\[n\](?!\[)', '', line)
            params_line = only_params(cleaned_line).rstrip('\n')
            params_only_list.append(params_line + '\n')

    with open(params_filename, 'w', encoding='utf-8') as params_file:
        for params in params_only_list:
            params_file.write(params)


def removeblank(input_path, output_path):
    with open(input_path, 'r', encoding='utf-8') as file:
        lines = file.readlines()
    ...
    for root, dirs, files in os.walk(input_directory):
        for file in files:
            if file.endswith('.msg'):
                ...
                msg_output_path = os.path.join('msgs', relative_path)
                params_output_path = os.path.join('msgparams', relative_path)

                os.makedirs(msg_output_path, exist_ok=True)
                os.makedirs(params_output_path, exist_ok=True)

                removeblank(input_path, input_path)
